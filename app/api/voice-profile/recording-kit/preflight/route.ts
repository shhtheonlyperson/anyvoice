...
export async function POST(req: NextRequest) {
  ...
  try {
    const preflight = await preflightVoiceProfileRecordingKit(manifest, profileId);
    return withAnyVoiceUserCookie(json({ preflight }), session);
  } catch (error) {
    const message = error instanceof Error ? error.message : "could not preflight recording kit";
