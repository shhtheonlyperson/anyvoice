...
export async function POST(req: NextRequest) {
  ...
  try {
    const check = await checkVoiceProfileRecordingKit(manifest, profileId);
    return withAnyVoiceUserCookie(json({ check }), session);
  } catch (error) {
    const message = error instanceof Error ? error.message : "could not check recording kit";
