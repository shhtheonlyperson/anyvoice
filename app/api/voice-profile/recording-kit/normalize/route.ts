...
export async function POST(req: NextRequest) {
  ...
  try {
    const normalization = await normalizeVoiceProfileRecordingKitAudio(manifest, profileId);
    return withAnyVoiceUserCookie(json({ normalization }), session);
  } catch (error) {
    const message = error instanceof Error ? error.message : "could not normalize recording kit audio";
